...
import {View, Text, Image, TouchableOpacity, TextInput, KeyboardAvoidingView, ActivityIndicator} from 'react-native';
...
export default RegisterScreen = props => {
  ...
  const [imageUri, setImageUri] = useState('http://s3.amazonaws.com/37assets/svn/765-default-avatar.png');
  const [expoToken, setExpoToken] = useState('');

  const handleKeyboardPress = key => {
    if (key == 'backspace') {
      confirmPin.length == 0 ? setPin(pin.slice(0, -1)) : setPin(pin.slice(0, -1));
    } else {
  ...
  const handleSubmit = () => {
    ...
    setIsLoading(true)
    if (pin == confirmPin) {
      Axios.post('https://linkwae.herokuapp.com/users/register', formData, config)
      .then(r => {
        console.log(r.data);
        setIsLoading(false);
      })
    ...
    <KeyboardAvoidingView style={styles.container} behavior='padding' enabled>
      ...
      <View style={styles.content}>
        ...
        <>
          ...
          <Text style={styles.subtitle}>Masukan foto serta nama kamu agar akun kamu dapat dikenali</Text>
          <View style={styles.imageSection}>
            <Image source={{uri: imageUri}} style={styles.image}></Image>
            <TouchableOpacity style={styles.addFotoButton} onPress={() => setIsTakePicture(true)}>
              <Text style={styles.addFotoButtonText}>Tambah Foto</Text>
            </TouchableOpacity>
          </View>
          <View style={styles.inputSection}>
            <TextInput onChangeText={(text) => setName(text)} style={styles.nameInput} placeholder='cth: Linka Jadid...' value={name}/>
            <Icon name='close-circle' style={styles.nameClearIcon} onPress={()=>setName('')}></Icon>
          </View> 
        </> : null
        }
        {currentPosition == 1 ? 
        <>
          ...
          <Text style={styles.subtitle}>Masukan dan  validasi alamat email kamu untuk memiliki akses ke reset PIN</Text>
          <View style={styles.inputSection}>
            <TextInput onChangeText={(text) => setEmail(text)} style={styles.nameInput} placeholder='Masukan Email Kamu' value={email}/>
          </View> 
        </> : null
        }
        {currentPosition == 2 ? 
        <>
          <Text style={styles.title}>{pin.length == 6 ? 'Confirmasi PIN Kamu' : 'Buat PIN'}</Text>
          <Text style={styles.subtitle}>{pin.length == 6 ? 'Masukan kembali PIN baru kamu' : 'Untuk keamana bertansaksi, masukan PIN kamu'}</Text>
          <View style={styles.pinSection}>
            {console.log(pin.length, '> 0', pin.length, '!= 6', (pin.length > 0 && pin.length != 6))}
            <View style={styles.singlePin}>{(confirmPin.length > 0 || (pin.length > 0 && pin.length != 6)) ? <View style={styles.filledPin}/> : <View/>}</View>
            <View style={styles.singlePin}>{(confirmPin.length > 1 || (pin.length > 1 && pin.length != 6)) ? <View style={styles.filledPin}/> : <View/>}</View>
            <View style={styles.singlePin}>{(confirmPin.length > 2 || (pin.length > 2 && pin.length != 6)) ? <View style={styles.filledPin}/> : <View/>}</View>
            <View style={styles.singlePin}>{(confirmPin.length > 3 || (pin.length > 3 && pin.length != 6)) ? <View style={styles.filledPin}/> : <View/>}</View>
            <View style={styles.singlePin}>{(confirmPin.length > 4 || (pin.length > 4 && pin.length != 6)) ? <View style={styles.filledPin}/> : <View/>}</View>
            <View style={styles.singlePin}>{(confirmPin.length > 5 || (pin.length > 5 && pin.length != 6)) ? <View style={styles.filledPin}/> : <View/>}</View>
          </View>
        </> : null
        }
      </View>
      { actions[currentPosition] != '' ?
        <TouchableOpacity style={styles.nextButton} onPress={() => setCurrentPosition(currentPosition + 1)}>
